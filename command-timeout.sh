...
CURRENT_TIME=$(date +%s)
NEXT_TIME=$(($CURRENT_TIME + TIMEOUT_SECONDS))
...
if [ "$ELAPSED" -ge "$TIMEOUT_SECONDS" ] || [ ! -f "$LOCK_FILE" ]; then
  ...
  # compute next run time in human readable form (GNU date). (Portable across macos and GNU)
  if date --version >/dev/null 2>&1; then
    # GNU date (Linux)
    NEXT_RUN_HR=$(date -d "@$NEXT_TIME" '+%Y-%m-%d %H:%M:%S')
  else
    # BSD date (macOS)
    NEXT_RUN_HR=$(date -r "$NEXT_TIME" '+%Y-%m-%d %H:%M:%S')
  fi
